module Bitgo
  module V1
    ...
    class Api
      ...
      def login(email: nil, password: nil, otp: nil)
        login_params = {
          email: email,
          password: password,
          otp: otp
      ...
      def unlock(otp: nil, duration_seconds: nil)
        unlock_params = {
          otp: otp,
      ...
      def add_keychain(xpub: nil, encrypted_xprv: nil)
      ...
      def list_labels_for_wallet(wallet_id: nil)
        call :get, '/labels/' + wallet_id
      end

      def set_label(wallet_id: nil, address: nil, label: nil)
        call :put, '/labels/' + wallet_id + '/' + address, { label: label }
      end

      def delete_label(wallet_id: nil, address: nil)
      ...
      # }
      def simple_create_wallet(passphrase: nil, label: nil, access_token: nil )
      ...
      def add_wallet(label: nil, m: nil, n: nil, keychains: nil, enterprise: nil)
      ...
      def create_address(wallet_id: nil, chain: nil, access_token: nil)
        @session_token = access_token
        call :post, '/wallet/' + wallet_id + '/address/' + chain
      end

      def send_coins_to_address(wallet_id: nil, address: nil, amount: nil, wallet_passphrase: nil, min_confirmations: nil, fee: nil, access_token: nil)
        @session_token = access_token
      ...
      def add_webhook(wallet_id: nil, type: nil, url: nil, confirmations: nil)
        add_webhook_params = {
          type: type,
          url: url,
      ...
      def remove_webhook(wallet_id: nil, type: nil, url: nil)
      ...
      def list_webhooks(wallet_id: nil)
      ...
      def encrypt(input: nil, password: nil)
        call :post, '/encrypt', { input: input, password: password }
      end

      def decrypt(input: nil, password: nil)
      ...
      def verify_address(address: nil)
